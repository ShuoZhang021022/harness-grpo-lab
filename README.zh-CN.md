...
仓库已实现工具库、轨迹控制、GRPO 核心、数据集、统计与绘图。项目提示词均为英文，并要求自然语言输出使用英文，同时保留规定的答案格式与程序接口。真实模型适配器和隔离执行后端尚未接入。模型配置选择自建部署的 Qwen3-8B 作为可训练主 agent 和固定的解题模型，并使用 Qwen3-32B 作为固定的相似性检查模型和离线参考答案审核模型。尚未下载或加载模型权重，也没有进行真实 LLM 训练或评估。数值测试样例和测试绘图不代表实验结果。
...
所选方案包含两种基础模型权重。主 agent 和解题模型均从 Qwen3-8B 开始，但解题模型必须保持初始参数固定，不能加载主 agent 训练得到的 LoRA 适配器。相似性检查模型和离线审核模型可以共用一个自建 Qwen3-32B 服务，分别使用独立请求、提示词和上下文。离线审核模型不得将参考答案传给在线 agent。同一个模型承担多个角色，不能作为其判断或答案正确的独立证据；隐藏测试、精确答案比较和参考答案核验仍按原方案执行。
